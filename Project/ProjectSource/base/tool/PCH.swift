...
let GDReloadMessageList = NSNotification.Name("GDReloadMessageList")
let GDReloadMeItem = NSNotification.Name("GDReloadMeItem")
...
let DDStatusBarHeight : CGFloat = DDDevice.type == .iphoneX ? 44 : 20
...
///调整页面的滑动根据键盘的弹出的位置。
func configKeyboard(subView: UIView, view: UIScrollView)  {
    
    if DDKeyBoardHandler.manager.keyboardY > 0 {
        DDKeyBoardHandler.manager.configContentSet(containerView: subView, inputView: view)
    }else {
        DDKeyBoardHandler.manager.zkqsetViewToBeDealt(containerView: subView, inPutView: view)
    }
