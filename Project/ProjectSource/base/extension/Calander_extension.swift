...
//        let localDate = date.addingTimeInterval(TimeInterval(inter))
    ...
    func getMonth(date: Date = Date.init()) -> Int {
        ...
        let calendar = Calendar.current
        var comp = calendar.dateComponents(Set<Calendar.Component>.init([Calendar.Component.year, Calendar.Component.month, Calendar.Component.day, .hour, .minute, .second]), from: date)
    ...
    ///获取现在是第几号。
    func getNowHour(date: Date = Date.init()) -> Int {
        
        var comp = self.dateComponents(Set<Calendar.Component>.init([Calendar.Component.hour]), from: date)
