...
class GDBaseWebVC: DDInternalVC , WKScriptMessageHandler ,  WKNavigationDelegate,WKUIDelegate {
    ...
    func layoutsubviews() {
        ...
        self.performLoadHtml()

        
    }
    
    func performLoadHtml()  {
        guard let urlStr = userInfo as? String else {
            //            mylog("webViewController对应的url字符串不存在\(self.showParameter)")
            return
        }
        
        if let token = DDRequestManager.share.DDToken {
            ...
            if let  urlstring = userInfo as? String  {
...
//        self.naviBar.attributeTitle = NSAttributedString.init(string: webView.title ?? "")
    ...
    func webView(_ webView: WKWebView, didFailProvisionalNavigation navigation: WKNavigation!, withError error: Error){
        ...
        let errorview = DDErrorView(superView: self.view, error: DDError.serverError("加载失败"))
        errorview.frame = CGRect(x: 0, y: DDNavigationBarHeight, width: self.view.bounds.width, height: self.view.bounds.height - DDNavigationBarHeight)
            errorview.automaticRemoveAfterActionHandle = {[weak self] in
            self?.performLoadHtml()
        }
//        let hemlSourcePath  = "Users/wy/desktop/testA.html"
//        let urlrequest = URLRequest.init(url: URL(fileURLWithPath: hemlSourcePath))
//        //        webView.load(urlrequest)
//        //        let imgUrl = "file:///Users/wy/Pictures/70AC4E6452681F85C254555A06E4F3F8.jpg"
//        //        let img = "<img src=\"\(imgUrl)\">"
//        //        webView.loadHTMLString("<p font=100 color = \"#C0C0C0\" >阿福说我改不了,看来要让它失望了</p> \(img)", baseURL: nil )
//        webView.evaluateJavaScript("alert(\"helloworld\")") { (any , error ) in
//            mylog(any)
//            mylog(error)
//        }
