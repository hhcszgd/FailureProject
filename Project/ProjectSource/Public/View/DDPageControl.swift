    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        if keyPath! == "contentOffset" {
            if let newPoint = change?[NSKeyValueChangeKey.newKey] as? CGPoint{
                ...
                }else{scrollViewWidth = SCREENWIDTH}
                if scrollViewWidth <= 0{scrollViewWidth = 1}
                var count : CGFloat = 1
                if let contentSizeWith = scrollView?.contentSize.width , let boundsWidth = scrollView?.bounds.size.width , contentSizeWith > 0 , boundsWidth > 0{
                    count = contentSizeWith / boundsWidth
                }
...
//                                mylog(count)
//                                mylog(sectionCount)
                                let temp1 = Int(count / CGFloat(sectionCount == 0 ? 1 : sectionCount))
                                let temp = temp1 == 0 ? 1 : temp1
//                                self.currentPage = index % Int(count / CGFloat(sectionCount == 0 ? 1 : sectionCount))
                                self.currentPage = index % temp
