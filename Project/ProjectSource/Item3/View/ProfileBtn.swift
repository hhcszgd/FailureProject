...
class ProfileBtn: UIButton {
    ...
    override func titleRect(forContentRect contentRect: CGRect) -> CGRect {
        ...
        if let image = self.currentImage {
            ...
            let titleWidth = self.currentTitle?.sizeWith(font: UIFont.systemFont(ofSize: 14), maxSize: CGSize.init(width: 80, height: 20)) ?? CGSize.init(width: 80, height: 20)
            let titleY = imagey + imageHeight + 3 * SCALE
